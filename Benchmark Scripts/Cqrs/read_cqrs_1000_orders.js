...
export function setup() {
    ...
    for (let i = 0; i < 1000; i++) {
        ...
        const res = http.post('http://localhost:5002/api/orders', JSON.stringify(order), {
    ...
    console.log('Setup complete, 1000 orders created.');
    sleep(10);
    return { users, products };
}

export default function (data) {
    const res = http.get('http://localhost:5002/api/orders');
